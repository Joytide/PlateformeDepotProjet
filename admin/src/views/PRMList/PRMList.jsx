...
class PRMList extends React.Component {
	...
	removePRM = id => () => {
		...
		AuthService.fetch(api.host + ":" + api.port + "/api/prm", {
			...
			.then(res => {
				if (res.ok) {
					let prms = [...this.state.prms]

					this.setState({ prms: prms.filter(prm => prm._id !== id) });
				}
				else
					throw res;
			})
	...
	render() {
		...
		if (this.state.loadingKeywords || this.state.loadingPrms) {
			...
			let data = this.state.prms.map((prm, i) => <PRMItem gray={i % 2 === 0} prm={prm} keywords={this.state.keywords} key={prm._id} removePRM={this.removePRM(prm._id)}></PRMItem>)
			return (
				<GridContainer>
...
export default withSnackbar(withStyles(styles)(PRMList));
